...
-   **Test coverage must stay above 80% (statements/branches/functions/lines) before a commit or a fix/feature is considered done.** Enforced by `coverageReporter.check.global` in `karma.conf.js`. Run `npm test` and confirm the coverage summary clears 80% on every metric — add specs for any newly introduced or changed code that drops it below that line.
